...
function NewPostForm(props) {
  function newPostFormSubmission(event) {
    ...
    props.onAddingPost({
      header: event.target.postHeader.value,
      body: event.target.postBody.value,
      id: v4(),
      voteCount: 0,
      timeStamp: currentDate,
      elapsedTime: new Moment(),
  ...
  return (
    <div>
      ...
      <form onSubmit={newPostFormSubmission}>
        <input type="text" name="postHeader" placeholder="post header" />
        <br />
        <input type="text" name="postBody" placeholder="write your post here" />
        <br />
